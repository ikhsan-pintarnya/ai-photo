...
interface WizardProps {
  ...
  updateFeatures: (key: keyof HeadshotFeatures, value: string) => void;
  onNext: () => void;
...
const VIBE_OPTIONS = [
  { id: 'Corporate Executive', label: 'Executive' },
  { id: 'Startup Founder', label: 'Tech / Startup' },
  { id: 'Modern Creative', label: 'Creative' },
  { id: 'Friendly Professional', label: 'Casual' },
];
          ...
          <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center border-b border-slate-50 pb-2">
                <span className="text-xs text-slate-500 font-bold">Persona</span>
                <span className="text-xs text-slate-900 font-black">{features.vibe}</span>
              </div>
              <div className="flex justify-between items-center border-b border-slate-50 pb-2">
  ...
  return (
    <div className="flex flex-col h-full max-w-5xl mx-auto w-full animate-fade-in space-y-12 pb-20">
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                        <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                            <Sparkles size={14} /> Style Persona
                        </h4>
                        <div className="flex flex-col gap-2">
                            {VIBE_OPTIONS.map((v) => (
                                <button
                                    key={v.id}
                                    onClick={() => updateFeatures('vibe', v.id)}
                                    className={`px-4 py-3 rounded-xl border-2 text-left text-xs font-bold transition-all
                                        ${features.vibe === v.id ? 'border-indigo-600 bg-indigo-50 text-indigo-900' : 'border-slate-100 bg-white hover:border-slate-200 text-slate-600'}
                                    `}
                                >
                                    {v.label}
                                </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
                ...
                <div className="space-y-3">
                    ...
                    <select 
                        ...
                        className="w-full bg-white border-2 border-slate-100 rounded-xl p-3 text-xs font-bold text-slate-900 outline-none focus:border-indigo-500"
                    >
        ...
        <button 
          ...
          disabled={!features.pose || !features.vibe || !features.attire}
